...
class BSTPreOrderClass implements IBinarySearchTreeThree {
    class Node {
        int data;
        Node left;
        Node right;
    ...
    private void preOrderOutputWrapper(Node root) {
        if (root != null) {
            if (isEmpty()) {
                throw new IllegalStateException("Trees is empty!");
            }
...
public class BSTPreOrder {
    ...
    public static void main(String[] args) {
        ...
        try (BufferedReader input =
            ...
            System.out.print("Trees size (max 15): ");
            treeSize = Integer.parseInt(input.readLine());
            if (treeSize > 15) {
                throw new IllegalStateException("Trees size shouldn't be greater than 15!");
            }
            BSTPreOrderClass tree = new BSTPreOrderClass(treeSize);
                ...
                switch (answer) {
                    case 1:
                        if (!tree.isFull()) {
                            ...
                            System.out.println("Trees is full!");
                        }
                        treeCreated = true;
                        break;
                    case 2:
    ...
    private static void treeIsNotCreated() {
        if (!treeCreated) {
            System.out.println("Trees is not created!");
        }
    }
}
